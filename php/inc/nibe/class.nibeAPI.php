...
class NibeAPI {
	...
	function authorize($CODE, $isRefresh = false)
	{
		...
		if ($response === false)
		{
			echo 'authorize(): Curl-Error: ' . curl_error($ch);
		}
		else
		{
			switch ($http_code = curl_getinfo($ch, CURLINFO_HTTP_CODE))
			{
				case 200:
					...
					if ($jsonResponse != NULL || strlen(trim($token)) > 0)
					{
						...
						echo 'authorize(): Could not decode json response: ' . $response . '\n';
					}
				break;
			
				default:
					echo 'authorize(): Unexpected HTTP-Code: ' . $http_code . "\n";
			}
		}
	...
	function readAPI($URI, $token, &$success = 'undefined')
	{
		...
		if ($response === false)
		{
			echo 'readAPI(): Curl-Error: ' . curl_error($ch);
		}
		else
		{
			switch ($http_code = curl_getinfo($ch, CURLINFO_HTTP_CODE))
			{
				...
				default:
					$data = "readAPI(): Unexpected HTTP-Code: " . $http_code . "<br />\nResponse:<br />\n" . $response;
			}
		}
	...
	function postPutAPI($URI, $method, $postBody, $token, &$success = 'undefined')
	{
		...
		if ($response === false)
		{
			echo 'postPutAPI(): Curl-Error: ' . curl_error($curl);
		}
		else
		{
			switch ($http_code = curl_getinfo($curl, CURLINFO_HTTP_CODE))
			{
				...
				default:
					$data = "postPutAPI(): Unexpected HTTP-Code: " . $http_code . "<br />\nResponse:<br />\n" . $response;
			}
		}
	...
	function save_token($token)
	{
		...
		{
			echo "save_token(): Could not save token.";
			die();
		}
	...
	function clear_token()
	{
		...
		{
			echo "clear_token(): Could not clear token.";
			die();
		}
	...
	function checkToken()
	{
		...
		{
			if ($this->debugActive) echo "checkToken(): Token needs update. Working on it...<br />\n";
			$token = $this->authorize($token->refresh_token, true);

			if ($token === false)
			{
				//$this->clear_token();
				echo "checkToken(): Failed to refresh token.<br />\n";
				return false;
			}

			$this->save_token($token);
			if ($this->debugActive) echo "checkToken(): Successfully refreshed token.<br />\n";
		}
